...
from llama_recipes.utils.loss import compute_raft_loss
    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            with profile(train_config,local_rank) as profile_context:
                for step, batch in enumerate(train_dataloader): #____________________________________________________________________________________________________________________
                    total_train_steps += 1
                    # stop when the maximum number of training steps is reached
                    ...
                    with autocast():
                        out = model(**batch)
                        loss=out.loss
                    if train_config.regression_aware:
                         print("regression_aware is True")
                         print(batch.keys())
                         print(batch["labels"].shape)
                         logits = out.logits
                         print(logits.shape)
                         loss = compute_raft_loss(logits,batch["labels"],tokenizer)
                    ...
                    if wandb_run:
                        if not train_config.enable_fsdp or rank==0:
                            ...
                            # Se raya biende si hago esto
                        # if train_config.run_validation and train_config.eval_steps > 0 and total_train_steps % train_config.eval_steps == 0:
                        #     torch.distributed.barrier()  # Synchronize all ranks before eval. I had a nccl error before..    and now as it seems

                        #     if rank == 0:
                        #         print("Running evaluation...")
                        #         eval_ppl, eval_loss, _, _ = evaluation(
                        #             model, train_config, eval_dataloader, local_rank, tokenizer, wandb_run
                        #         )
                        #         wandb_run.log({
                        #             'eval/step': total_train_steps,
                        #             'eval/loss': eval_loss,
                        #             'eval/perplexity': eval_ppl,
                        #         }, step=total_train_steps)

                        #     torch.distributed.barrier()  # Wait for rank 0 to finish eval
